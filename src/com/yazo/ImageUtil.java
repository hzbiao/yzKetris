...
public class ImageUtil {
	 ...
	 */
	public static Image effect_light_contrast(Image src, double contrast,
			int light) {
		int srcW = src.getWidth();
		int srcH = src.getHeight();
		int[] srcPixels = getPixels(src);
...
//		int argb;
	...
	public Image effect_image_add_image(Image src, Image image, int x_pos,
		...
		src.getRGB(srcRgbdata, 0, src.getWidth(), 0, 0, src.getWidth(),
				src.getHeight());
	 ...
	 * 图片上添加字符
	 */
	public static Image effect_image_add_str(Image src, String str, int x_pos,
			int y_pos) {
		Image temp = Image.createImage(src.getWidth(), src.getHeight());
		Graphics g = temp.getGraphics();
